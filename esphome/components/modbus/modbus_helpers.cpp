...
// StaticVector::capacity() postdates 2026.8.x; read N directly so this builds on the released core.
template<typename V> struct pdu_capacity;
template<typename T, size_t N> struct pdu_capacity<StaticVector<T, N>> { static constexpr size_t value = N; };
...
template<typename Pdu> static Pdu build_write_registers_pdu(uint16_t start_address, std::span<const uint16_t> values) {
  constexpr auto max_registers = static_cast<uint16_t>((pdu_capacity<Pdu>::value - WRITE_MULTIPLE_HEADER_SIZE) / 2);
...
static_assert((pdu_capacity<PduBuffer>::value - WRITE_MULTIPLE_HEADER_SIZE) / 2 == MAX_NUM_OF_REGISTERS_TO_WRITE,
              "a full-frame PDU must hold exactly MAX_NUM_OF_REGISTERS_TO_WRITE registers");
static_assert((pdu_capacity<WriteFewRegistersPdu>::value - WRITE_MULTIPLE_HEADER_SIZE) / 2 == MAX_FEW_REGISTERS,
